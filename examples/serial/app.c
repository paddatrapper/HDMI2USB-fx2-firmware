...
void mainInit(void) {
    ...
    // This is only necessary for cases where you want to load firmware into the
    // RAM of an FX2 that has already loaded firmware from an EEPROM. It should
    // definitely be removed for firmwares which are themselves to be loaded
    // from EEPROM.
#ifndef EEPROM
    RENUMERATE_UNCOND();
#endif
...
void TD_Init(void)
{
     ...
     *                      Selects b/w internal and external sources:
     *                      0 = external, 1 = internal
     *          BIT 6   : 3048MHZ, Internal FIFO/GPIF Clock Frequency. 
     *                      Selects b/w the 30- and 48-MHz internal clock:
     *                      0 = 30 MHz, 1 = 48 MHz.
     *                      This bit has no effect when IFCONFIG.7 = 0.
     *          BIT 5   : IFCLKOE, IFCLK pin output enable. 
     *                      Output enable for the internal clock source:
     *                      0 = disable(tristate), 1 = enable(drive).
     *                      This bit has no effect when IFCONFIG.7 = 0.
     *          BIT 4   : IFCLKPOL, Invert the IFCLK signal. 
     *                      Inverts the polarity of the interface clock
     *                      (whether it’s internal or external):
     *                      0 = normal, 1 = inverted.
     *          BIT 3   : ASYNC, FIFO/GPIF Asynchronous Mode
     *                      When ASYNC=0, the FIFO/GPIF operate synchronously:
     *                      a clock is supplied either internally or externally
     *                      on the IFCLK pin; the FIFO control signals function
     *                      as read and write enable signals for the clock
     *                      signal.
     *                      When ASYNC=1, the FIFO/GPIF operate asynchronously:
     *                      no clock signal input to IFCLK isrequired; the FIFO
     *                      control signals function directly as read and write
     *                      strobes.
     *          BIT 2   : GSTATE, Drive GSTATE [2:0] on PORTE [2:0].
     *                      When GSTATE=1, three bits in Port E take on the
     *                      signals shown in Table 15-6. The GSTATE bits, which
     *                      indicate GPIF states, are used for diagnostic
     *                      purposes. 
     *          BIT 1-0 : IFCFG, Select Interface Mode (Ports, GPIF, or Slave
     *                    FIFO)
     *                      Required to be 0b11 for Slave FIFO. See Page 15-16
     *                      TRM
     */
     
    // Internal Clock, 48MHz, IFCLK output enable to pin, Normal Polarity, 
    // Synchronous FIFO, Nothing to do with GSTATE, Set interface mode to Slave
    // FIFO.
     ...
     *      Structure:
     *          BIT 7   : VALID, Activate an Endpoint. Set VALID=1 to activate
     *                    an endpoint, and VALID=0 to de-activate it.
     *          BIT 6   : Unused
     ...
     * 
     *      Structure:
     *          Bit 7   : VALID, Set VALID=1 to activate an endpoint, and
     *                    VALID=0 to de-activate it
     ...
     *                      Note: Endpoints 4 and 8 can only be 512 bytes.
     *                      Endpoints 2 and 6 are selectable.
     *          BIT 2   : Unused
